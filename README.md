...
That last point is the one that matters, so it is worth being precise about what is and is
not borrowed.

- The name is used **nominatively**, to say what this extension works with. There is no
  other way to describe the problem it solves.
- The client menu on the guidance page is drawn from scratch in HTML, CSS and SVG, in
  `src/help.html`. It is a simplified diagram that exists so the page can fill in the
  tenant's own account details at runtime, which a screenshot could not do. No image file
  from Tailscale is bundled, linked or copied.
- The extension icon is generated by `tools/make_icons.py`. It uses a nine-dot grid, which
  is the visual language of Tailscale's mark, with an H lit rather than their t. It is a
  distinct mark and it is ours, but it is deliberately reminiscent, and that is a choice
  made with open eyes rather than an accident.

For support with Tailscale itself, go to Tailscale. Issues raised here about the Tailscale
client will be closed and pointed there.
